...
while (g>0.001)
{
  ...
  Jyx11 <- pdf(Weibull(a,b),x1)/pdf(Normal(0,1),y1)
  Jyx22 <- BiCopPDF(U1,U2,family=34,par=-1.69)*pdf(Uniform(8000,260000),x2)/pdf(Normal(0,1),y2)
  Jyx33 <- BiCopPDF(U12,U32,family=5,1.35)*BiCopPDF(U2,U3,family=33,-1.12)*pdf(Gamma(1.19,1/0.27),x3)/pdf(Normal(0,1),y3)
  Jyx44 <- pdf(Normal(1,0.033),x4)/pdf(Normal(0,1),y4)
  Jyx55 <- pdf(Normal(15,5),x5)/pdf(Normal(0,1),y5)
  
  # cari Jyx12 pake numerical differentiation (turunan terhadap x1)
  h       <-  0.01
  U1h     <-  cdf(Weibull(a,b),x1+h)
  ...
  h        <- 10
  U2h      <- cdf(Uniform(8000,260000),x2+h)
